...
bool PatchMemory(const HookPatch& patch)
{
	...
	if (error)
	{
		...
		MessageBoxA(nullptr, message.c_str(), "xwing_series_hook_main (DInput.dll)", MB_OK | MB_ICONERROR);
		return false;
	}
...
public:
	...
	{
		char filename[4096];
		std::string workingDir = std::experimental::filesystem::current_path().string();
		if (GetModuleFileName(nullptr, filename, sizeof(filename)) != 0)
		{
			workingDir = filename;
			workingDir.erase(workingDir.rfind("\\"));
		}

		for (const auto& file : std::experimental::filesystem::directory_iterator(workingDir))
		{
...
int HookError(int call, int* params)
{
	...
	MessageBoxA(nullptr, text.c_str(), "xwing_series_hook_main (DInput.dll)", MB_OK | MB_ICONERROR);
